...
import com.zhaoxk_job.service.StudentService;
...
//			session.setAttribute("xuehao", stu.getXuehao());
   ...
   //****************************************************************************************
     public String modifyPassword()  {
 		...
 		System.out.println("学号�?" + student.getXuehao()+"密码�?" +student.getPassword()+ ",确认密码�?" + confirmPassword);
   ...
   //****************************************************************************************
	...
	public void validate(){
    	...
    	{
    		addFieldError("password", "用户密码必须�?6�?20位之�?!");//返回“INPUT”�?�辑视图
    	}
    	if(student.getXuehao()!=null)
    		if (student.getXuehao().length()!=11)
    	{
    		addFieldError("xuehao", "学号必须�?11位的数字!");//返回“INPUT”�?�辑视图
    	}
    	if(student.getStudentName()!=null)
		    if(student.getStudentName().length()==0)
    	{
    		addFieldError("studentName", "姓名不能为空!");//返回“INPUT”�?�辑视图
    	}
    	if(student.getBanji()!=null)
	         if(student.getBanji().length()==0)
    	{
    		addFieldError("banji", "班级不能为空!");//返回“INPUT”�?�辑视图
    	}
    	if(student.getBirthday()!=null)
        if(student.getBirthday().length()==0)
    	{
    		addFieldError("birthday", "生日不能为空!");//返回“INPUT”�?�辑视图
    	}
    }
